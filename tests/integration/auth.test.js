...
describe('Auth routes', () => {
  ...
  describe('POST /v1/auth/logout', () => {
    test('should return 204 if refresh token is valid', async () => {
      ...
      const refreshToken = tokenService.generateToken(userOne._id, expires, TOKEN_TYPES.REFRESH);
  ...
  describe('POST /v1/auth/refresh-tokens', () => {
    ...
    test('should return 401 error if refresh token is signed using an invalid secret', async () => {
      await insertUsers([userOne]);
      const expires = moment().add(config.jwt.refreshExpirationDays, 'days');
      const refreshToken = tokenService.generateToken(userOne._id, expires, TOKEN_TYPES.REFRESH, 'invalidSecret');
      await tokenService.saveToken(refreshToken, userOne._id, expires, TOKEN_TYPES.REFRESH);
  ...
  describe('POST /v1/auth/reset-password', () => {
    test('should return 204 and reset the password', async () => {
      ...
      const resetPasswordToken = tokenService.generateToken(userOne._id, expires, TOKEN_TYPES.RESET_PASSWORD);
      ...
      const dbResetPasswordTokenCount = await Token.countDocuments({ user: userOne._id, type: TOKEN_TYPES.RESET_PASSWORD });
...
describe('Auth middleware', () => {
  ...
  test('should call next with unauthorized error if access token is generated with an invalid secret', async () => {
    await insertUsers([userOne]);
    const expires = moment().add(config.jwt.accessExpirationMinutes, 'minutes');
    const accessToken = tokenService.generateToken(userOne._id, expires, TOKEN_TYPES.ACCESS, 'invalidSecret');
  ...
  test('should call next with unauthorized error if access token is expired', async () => {
    await insertUsers([userOne]);
    const expires = moment().subtract(1, 'minutes');
    const accessToken = tokenService.generateToken(userOne._id, expires, TOKEN_TYPES.ACCESS);
    const req = httpMocks.createRequest({ headers: { Authorization: `Bearer ${accessToken}` } });
    const next = jest.fn();
