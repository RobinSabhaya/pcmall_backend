...
export const checkout = async (request:FastifyRequest, reply:FastifyReply) => {
  try {
    ...
    const { shippingAddress, currency, items, shippoShipmentId, rateObjectId, cartIds } = request.body as CheckoutSchema ;
    ...
    const checkoutUrl = await handlePayment(config.paymentGateway.paymentProvider!).createCheckoutSession({
      user,
      items,
      shippingAddress,
      currency,
