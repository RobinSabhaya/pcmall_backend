...
declare module 'fastify' {
    interface FastifyInstance {
        authorizeV1: (...requiredRights: string[]) => (request: FastifyRequest, reply: FastifyReply) => Promise<void>;
    }
    ...
    fastify.decorate('authorizeV1', (...requiredRights: string[]) => {
        return async function (request: FastifyRequest, reply: FastifyReply) {
            try {
                ...
                // ✅ Role check
                const roleData = await findOneDoc<IRole>(MONGOOSE_MODELS.ROLE, { role: { $in: user.roles } });
                if (!roleData) {
                ...
                const permissionExists = await findOneDoc<IPermission>(MONGOOSE_MODELS.PERMISSION, { slug: requireRights });
                if (permissionExists) {
                    const accessPermission = await findOneDoc(
