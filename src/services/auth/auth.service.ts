...
export const loginUserWithEmailAndPassword = async (email: string, password: string): Promise<IUser | null> => {
...
export const resetPassword = async (resetPasswordToken: string, newPassword: string): Promise<void> => {
  try {
    ...
    const user = await findOneDoc<IUser>(MONGOOSE_MODELS.USER, { _id: resetPasswordTokenDoc?.user });
    if (!user) {
      throw new Error();
    }
    await findOneAndUpdateDoc<IUser>(MONGOOSE_MODELS.USER, { _id: user?.id }, { password: newPassword });
...
export const verifyEmail = async (verifyEmailToken: string): Promise<void> => {
  try {
    ...
    await findOneAndUpdateDoc<IUser>(MONGOOSE_MODELS.USER, { _id: user?.id }, { isEmailVerified: true });
  } catch (error) {
