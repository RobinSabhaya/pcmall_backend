...
export const deleteBrand = async (filter: Partial<createUpdateBrandSchema>, options?: IOptions): Promise<{
  message: string;
  brandData: IProductBrand | null
}> => { 
  const { brandId } = filter;
    ...
    brandData = await findOneAndDeleteDoc<IProductBrand>(MONGOOSE_MODELS.PRODUCT_BRAND, { _id: brandId });
